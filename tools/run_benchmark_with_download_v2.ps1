...
Примеры:
  ...
  .\run_benchmark_with_download.ps1 -NoDownload -PullSet cpu
  .\run_benchmark_with_download.ps1 -NoDownload -Model qwen2.5-coder:3b -Full
...
param(
    ...
    # Если указан, скачивание моделей пропускается, сразу запускается benchmark.
    # В benchmark будут переданы модели из -Model или выбранного -PullSet.
    [switch]$NoDownload,
...
if ($NoDownload) {
    Write-Log "Запуск: benchmark без скачивания моделей" "INFO" ([ConsoleColor]::Cyan)
}
else {
...
Write-Log "PullSet: $PullSet | Retries: $Retries | NoDownload: $NoDownload" "INFO" ([ConsoleColor]::Gray)
Write-Log "============================================================" "INFO" ([ConsoleColor]::Gray)
...
if ($NoDownload) {
    Write-Host ""
    Write-Host "============================================================" -ForegroundColor DarkGray
    Write-Host "СКАЧИВАНИЕ ПРОПУЩЕНО" -ForegroundColor Yellow
    Write-Host "============================================================" -ForegroundColor DarkGray
    Write-Log "PULL SKIPPED | benchmark будет запущен без скачивания" "INFO" ([ConsoleColor]::Yellow)

    foreach ($m in $ModelsToPull) {
        [void]$OkModels.Add($m)
    }

    Write-Host "Модели, которые будут переданы в benchmark:" -ForegroundColor Cyan
